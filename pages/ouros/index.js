...
const Index = () => {
    ...
    const submit = (e)=>{if(passwordRef.current.value==data[level][1]){alert(data[level][2]);if (data[level+1][1]!=""){setLevel(level+1)}else{router.push('/ourosterm');setLevel(1);return;}setIsLoading(true);timeout=setTimeout(()=>{setIsLoading(false);},2000);passwordRef.current.value="";}
        else{alert(data[level][4]);}};return (<div className='background'><SVG />{!isLoading ? <div className='center-main'><h1 className='level'>Code {level} for termination of OUROS</h1><div className='label'>{data[level][3]}</div><input className='password' type="password" placeholder='Password' ref={passwordRef}></input><button className='submit' onClick={submit}>Advance</button> </div>
